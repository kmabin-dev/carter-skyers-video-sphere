...
def test_only_first_8_fans_log_info(tmp_path, caplog, monkeypatch):
    ...
    info_msgs = [r for r in caplog.records if r.levelno == logging.INFO and 'sent shard -> shared buffer' in r.getMessage()]
    debug_msgs = [r for r in caplog.records if r.levelno == logging.DEBUG and 'sent shard -> shared buffer' in r.getMessage()]
