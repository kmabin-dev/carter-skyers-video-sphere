...
def temp_file():
    ...
    tmp = tempfile.NamedTemporaryFile(
        delete=False, dir=str(config.TEMP_DIR), suffix=".mp4"
    )
...
def test_register_failed_temp(shared_buffer, temp_file):
    ...
    assert (
        len(failed_temps_again) == 0
    ), "Failed temps list should be empty after clear"
...
def test_vj_capacity_flag(shared_buffer):
    ...
    assert (
        not shared_buffer.vj_has_all_shards.value
    ), "Flag should start as False"

    # Set to True (simulating DJ completion)
    shared_buffer.vj_has_all_shards.value = True
    assert (
        shared_buffer.vj_has_all_shards.value
    ), "Flag should be True after setting"
...
def test_bounded_capacity(shared_buffer):
    ...
    result = shared_buffer.put_shard(
        "fan_overflow", "path_overflow", timeout=0.1
    )
...
def test_qsize(shared_buffer):
    ...
    assert (
        size == 1
    ), f"Buffer should have 1 item after consuming one, got {size}"
