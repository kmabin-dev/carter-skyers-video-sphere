...
def main():
    parser = argparse.ArgumentParser(
        description="Verify shard files exist and are readable"
    )
    parser.add_argument(
        "--count",
        type=int,
        default=int(os.environ.get("EXPECTED_SHARD_COUNT", "128")),
    ...
    if missing or unreadable:
        if missing:
            print(
                f"Missing {len(missing)} shard files; first few: {missing[:10]}",
                file=sys.stderr,
            )
        if unreadable:
            print(
                f"Unreadable {len(unreadable)} shard files; first few: {unreadable[:10]}",
                file=sys.stderr,
            )
        return 2

    print(
        f"All {args.count} shard files are present and readable in {config.SHARDS_DIR}"
    )
    return 0
