...
def file_hash(file_path):
    ...
    try:
        ...
        logger.error(
            "Unable to open %s exception=%s", file_path, type(e).__name__
        )
        quit(-1)
...
def concat(name, *input_video_file_paths):
    ...
    for i in range(n):
        ...
        if not os.path.exists(str(input_video_file_path)):
            ...
            logger.warning(
                "Input file not found (continuing): %s", input_video_file_path
            )
    print(f"\tname        : {name}")
    print("\tstatus      : processing...", end="")
    output_file = temp_file_path(name, ".mp4")
...
def play(file_path):
    ...
    events.event_attach(
        vlc.EventType.MediaPlayerEndReached, end_reached_cb, params
    )
    events.event_attach(
